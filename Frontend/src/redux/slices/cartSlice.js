...
//helper fun to load cart from local storage 
const loadCartFromStorage=()=>{
    const storedCart=localStorage.getItem("cart");
    return storedCart?JSON.parse(storedCart):{products:[]};
};

//helper fun to save cart to localstorage
...
export const removeFromCart = createAsyncThunk("cart/removeFromCrt",async({productId,size,color,guestId,userId},{rejectWithValue})=>{
    try {
...
export const mergeCart = createAsyncThunk("cart/mergeCart",async({guestId,userId},{rejectWithValue})=>{
    try {
        const response=await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/cart/marge`,{guestId,user},{
            headers:{
